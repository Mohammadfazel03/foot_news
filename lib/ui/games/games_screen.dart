...
class _GamesScreenState extends State<GamesScreen>
  ...
  @override
  void initState() {
    _currentPosition = BlocProvider.of<tab_bloc.TabBloc>(context).state.tab.position;
    _currentCount = BlocProvider.of<tab_bloc.TabBloc>(context).state.tab.count;
    _tabController =
        TabController(length: _currentCount, vsync: this, initialIndex: _currentPosition);
      ...
      builder: (context, state) {
        return Scaffold(
          ...
          body: Column(children: <Widget>[
            Expanded(
              child: TabBarView(controller: _tabController, children: getWidgets(state.tab.items)),
            ),
          ]),
        );
      },
  ...
  List<Widget> getWidgets(Map<DateTime, String> items) {
    ...
    items.forEach((key, value) {
      screens.add(BlocProvider(
          create: (context) => getIt<FixtureTabBloc>(param1: key),
  ...
  onPositionChange() {
    if (!_tabController.indexIsChanging) {
      ...
      if (_tabController.index == _tabController.length - 1) {
        BlocProvider.of<tab_bloc.TabBloc>(context)
            .add(tab_bloc.TabEvent(addToEnd: true, currentPosition: _tabController.index));
      } else if (_tabController.index == 0) {
        BlocProvider.of<tab_bloc.TabBloc>(context)
            .add(tab_bloc.TabEvent(addToEnd: false, currentPosition: _tabController.index));
      }
    }
  }
